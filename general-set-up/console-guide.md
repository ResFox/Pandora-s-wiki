# 🕹️ Console Guide

{% hint style="success" %}
To use Skynet on consoles, you need to install an application that allows you to remotely view your console's screen on your PC. There are several options for this. Any of them will work fine in most cases.
{% endhint %}

### PlayStation Device

<figure><img src="https://2000999588-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fxr02iQbBt5IsdQ3g6zQo%2Fuploads%2F7kz01c8NgRq1ruOSlG4I%2Fimage.png?alt=media&amp;token=d52e3e09-4690-49ec-b80b-4f20f8fc5039" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
For the Play Station we recommend 2 programs, Chiaki and PlayStation Remote
{% endhint %}

Go to your PlayStation Settings and enable Remote Play

<figure><img src="https://2000999588-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fxr02iQbBt5IsdQ3g6zQo%2Fuploads%2FkeIP7krzlvo0UkKVP6Qf%2Fimage.png?alt=media&amp;token=02625a41-695e-4607-a56c-855435949352" alt=""><figcaption></figcaption></figure>

#### Option 1 Chiaki:

Download Link:

{% embed url="https://git.sr.ht/~thestr4ng3r/chiaki/refs/download/v2.2.0/Chiaki-v2.2.0-Windows-x86_64.zip" %}

Recommended video tutorial:

{% embed url="https://youtu.be/HDagTesM6NY" %}

Auth PSN API:

{% embed url="https://trinket.io/embed/python3/52183a157e?outputOnly=true&runOption=run&start=result&showInstructions=true" %}

{% embed url="https://auth.api.sonyentertainmentnetwork.com/2.0/oauth/authorize?service_entity=urn:service-entity:psn&response_type=code&client_id=ba495a24-818c-472b-b12d-ff231c1b5745&redirect_uri=https://remoteplay.dl.playstation.net/remoteplay/redirect&scope=psn:clientapp&request_locale=en_US&ui=pr&service_logo=ps&layout_type=popup&smcid=remoteplay&prompt=always&PlatformPrivacyWs1=minimal&" %}

#### Option 2 PS Remote Play

Download Link:

{% embed url="https://remoteplay.dl.playstation.net/remoteplay/lang/en/" %}

Recommended video tutorial:

{% embed url="https://youtu.be/m3_2ipoZSAk" %}

### XBOX Device

<figure><img src="https://2000999588-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fxr02iQbBt5IsdQ3g6zQo%2Fuploads%2Fh75ALdx74NoPRUxYyjmW%2Fimage.png?alt=media&amp;token=69ebe0d9-6db2-4a53-a89e-9c56f50a6267" alt=""><figcaption></figcaption></figure>

Recommended video tutorial:

{% embed url="https://www.youtube.com/shorts/I4mIBKD7CZA?feature=share" %}

{% embed url="https://youtu.be/_xCy356grSQ" %}

AFTER DONE THE CONSOLE SETUP FOLLOW THE CONTROLLE PC GUIDE:

{% hint style="info" %}
REMEMBER THAT YOUR CONTROLLER MUST BE CONNECTED TO THE PC, NOT THE CONSOLE
{% endhint %}
